...
var HIDE_NAV_PARAM_KEY = "hidenav";
...
jQuery(function() {
    if (urlParams[HIDE_NAV_PARAM_KEY]) {
      jQuery("#navbar").hide();
    }
});
...
function filterData(data) {
    ...
    if (!data || data.length === 0) {
        ...
        if (!topicIds) {
            ...
            var queryStart = 'http://' + BASE_SERVER + "/pressgang-ccms/rest/1/topics/get/json/query;topicIds=";
...
function fixUpCompileTime(timeString) {
    ...
    var lastBuildMatch = /(\d{4}-\d{2}-\d{2}T)\s?(\d{1,2})(:\d{1,2}:\d{2}\.\d{3}\+\d{4})/.exec(timeString);
        ...
        _.each(thisProductAndVersionBooks, function (bookElement, index, list) {
            ...
            if (isTranslated()) {
                ...
                if (buildOk) {
                    ...
                    if (lastBuildMatch) {
                        lastBuild = fixUpCompileTime(decodeURIComponent(lastBuildMatch[1]));
                    }
